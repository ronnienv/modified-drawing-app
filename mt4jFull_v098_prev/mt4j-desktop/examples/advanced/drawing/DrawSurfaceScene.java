					...
					public boolean processGestureEvent(MTGestureEvent ge) {
						if(brushScale < 6){
							...
							float distance = (float) Math.sqrt(
									(ze.getFirstCursor().getCurrentEvtPosX()
											-ze.getSecondCursor().getCurrentEvtPosX())
											*(ze.getFirstCursor().getCurrentEvtPosX()
											-ze.getSecondCursor().getCurrentEvtPosX()) 
											+ (ze.getFirstCursor().getCurrentEvtPosY()
											-ze.getSecondCursor().getCurrentEvtPosY())
											*(ze.getFirstCursor().getCurrentEvtPosY()
											-ze.getSecondCursor().getCurrentEvtPosY()));
							
							//enlarge brush size
							if (distance > 800 && brushScale < 3)
								brushScale += .01;
							//decrease brush size
							else if(distance < 250 && brushScale > .2) {
								brushScale -= .03;
							}
							zoomed = true;
...
//					System.out.println("Pos: " + pos);
	...
	public void setBrush(AbstractShape brush, Boolean lineMode){
